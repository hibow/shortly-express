...
describe('', function () {
  ...
  describe('Sessions and cookies', function () {
    ...
    it('cookie should be expired after logout(1sec later) ', function (done) {
      addUser(function (err, res, body) {
        if (err) { return done(err); }
        var cookies = cookieJar.getCookies('http://127.0.0.1:4568/');
        var cookieValue = cookies[0].value;
        ...
        requestWithSession('http://127.0.0.1:4568/logout', function (error, response, resBody) {
          ...
          setInterval( function() {
            cookies = cookieJar.getCookies('http://127.0.0.1:4568/');
          }, 2000);
  ...
  describe('Link creation:', function () {
    ...
    describe('With previously saved urls:', function () {
      ...
      it('Should increase click count to 1', function (done) {
        var options = {
          'method': 'GET',
          'uri': 'http://127.0.0.1:4568/' + link.code
        };
        requestWithSession(options, function (error, res, body) {
          ...
          db.query('SELECT * FROM links WHERE url = "http://www.google.com/"', function (err, links) {
            ...
            if (err) { return done(err); }
            console.log(links);
            if (links[0] && links[0]['visits']) {
              visits = links['0']['visits'];
            }
      ...
      it('Delete all of the links on the links page', function (done) {
        var options = {
          'method': 'DELETE',
          'uri': 'http://127.0.0.1:4568/links'
        };
        requestWithSession(options, function (error, res, body) {
          ...
          expect(body).to.not.include('"title":"Google"');
          expect(body).to.not.include('"code":"' + link.code + '"');
